//管理员操作表格
import { deleteChartUsingPOST, listChartByPageUsingPOST } from '@/services/lilyBI/ChartController';
          ...
          <List.Item key={item.id}>
            <Card style={{ width: '100%' }}>
              ...
              <>
                {item.chartStatus === 0 && (
                  <>
                    <Result
                      status="warning"
                      title="排队中...."
                      subTitle={item.executorMessage ?? '系统繁忙，请稍后重试'}
                    />
                    <Row>
                      <Col push={17}>
                ...
                {item.chartStatus === 3 && (
                  <>
                    <Result status="info" title="图表生成中...." subTitle={item.executorMessage} />
                  </>
                )}
                {item.chartStatus === 1 && (
                  <>
                ...
                {item.chartStatus === 2 && (
                  <>
                    <Result status="error" title="图表生成失败" subTitle={item.executorMessage} />
                    <Row justify="end">
